...
const getAllBusinessServiceByBusinessId = async (
...
const getAllBusinessService = async (query: Record<string, unknown>) => {
  const businessServiceQuery = new QueryBuilder(
    Service.find({}).select('businessId').populate('businessId'),
    query,
  )
    .search([''])
    .filter()
...
const getAllAdminServiceByBusinessId = async (businessId: string) => {
  const businessIdx = new mongoose.Types.ObjectId(businessId);
  const service = await Service.find({ businessUserId: businessIdx });
  return service;
};

const getAllAdminByService = async (query: Record<string, unknown>) => {
  const businessServiceQuery = new QueryBuilder(
...
const deletedBusinessServiceService = async (
  id: string,
  businessId: string,
) => {
  const existingBusinessService = await Service.findById(id);
