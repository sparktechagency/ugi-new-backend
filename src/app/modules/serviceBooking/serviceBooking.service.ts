...
// const businessmanCancelBookingService = async (id:string, businessId:string)=>{
...
//       message: `Booking Cancelled Successfully! Refund is 100% of the deposit.`,
//       type: 'success',
//     };
...
const businessmanCancelBookingService = async (
  id: string,
  businessId: string,
) => {
  const session = await mongoose.startSession();
  ...
  try {
    ...
    // Commit transaction
    await session.commitTransaction();
    await session.endSession(); // Added await
