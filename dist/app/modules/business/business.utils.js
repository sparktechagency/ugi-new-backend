...
exports.generateNewTimeSlot = generateNewTimeSlot;
exports.getDayDate = getDayDate;
function generateAvailableSlots({ startTime, endTime, startBreakTime, endBreakTime, bookings, duration, minimumSlotTime, bookingBreak, }) {
    console.log({
        startTime,
        endTime,
        startBreakTime,
    ...
    function convertToDate(time) {
        ...
        // console.log({ time });
        const [timeStr, period] = time === null || time === void 0 ? void 0 : time.split(' ');
        // console.log({ timeStr });
        // console.log({ period });
        const [hours, minutes] = timeStr === null || timeStr === void 0 ? void 0 : timeStr.split(':').map(Number);
        // console.log({ hours });
        // console.log({ minutes });
    ...
    function generateTimeSlots(start, end, minSlotDuration) {
        ...
        while (currentTime < endTime) {
            ...
            // console.log({ nextTime });
            if (nextTime <= endTime) {
                slots.push(`${currentTime.getHours() < 13 ? currentTime.getHours() : currentTime.getHours() - 12}:${String(currentTime.getMinutes()).padStart(2, '0')} ${currentTime.getHours() < 12 ? 'AM' : 'PM'}`);
            }
    ...
    const availableSlots = allSlots.filter((slot, i) => {
        ...
        const isBooked = bookedSlots.find((booking) => (slotStart >= booking.start && slotStart < booking.end) ||
        ...
        const violatesBreak = bookedSlots.find((booking) => slotStart >= booking.end &&
...
function isTimeOverlap(mentorStart, mentorEnd, queryStart, queryEnd) {
    ...
    return (mentorStartMinutes < queryEndMinutes && mentorEndMinutes > queryStartMinutes);
}
/// filter query business
function generateNewTimeSlot(timeSlots) {
    ...
    const earliestStart = parsedSlots.reduce((earliest, current) => new Date(`1970-01-01T${convertTo24Hour(current.start)}`) <
        new Date(`1970-01-01T${convertTo24Hour(earliest.start)}`)
        ? current
        : earliest).start;
    // Find the latest end time
    const latestEnd = parsedSlots.reduce((latest, current) => new Date(`1970-01-01T${convertTo24Hour(current.end)}`) >
